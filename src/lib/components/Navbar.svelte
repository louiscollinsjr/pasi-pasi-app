<script lang="ts">
  ...
  async function logout() {
    ...
    if (error) {
      ...
      // The onAuthStateChange listener in AuthForm will clear the user store
      await goto('/login');
    }
  }
</script>
...
<nav class="fixed top-0 left-0 right-0 z-50 bg-white">
  <div class="flex items-center justify-between px-4 sm:px-12 py-6">
      ...
      >
        <div class="relative w-[18px] h-[18px]">
          ...
          <svg 
            ...
            class="text-gray-600 absolute inset-0 transition-all duration-300 ease-in-out {sidebarOpen ? 'opacity-0 rotate-180 scale-90' : 'opacity-100 rotate-0 scale-100'}"
          >
          ...
          <!-- Visible sidebar icon -->
          <svg 
            width="18" 
            viewBox="0 0 24 24" 
            fill="none" 
            xmlns="http://www.w3.org/2000/svg" 
            class="text-gray-600 absolute inset-0 transition-all duration-300 ease-in-out {sidebarOpen ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-180 scale-90'}"
          >
    ...
    <div class="flex items-center gap-4">
      {#if $userStore}
        <span class="text-sm text-gray-600">{$userStore.email}</span>
        <Button on:click={logout} variant="outline" size="sm">Log out</Button>
      {:else}
        <a href="/login" tabindex="0" role="button"
          class="text-lg rounded-full font-medium bg-gray-100 text-black px-8 py-4 focus:outline-none focus:ring-2 focus:ring-primary transition-colors hover:bg-gray-200 block text-center">
          Log in
        </a>
      {/if}
    </div>
